...
public class AsyncMethodProcessor implements ClassHolderTransformer {
    ...
    private void generateLowLevelCall(MethodHolder method, int suffix) {
        ...
        if (!method.hasModifier(ElementModifier.STATIC)) {
            constructorInvocation.getArguments().add(instanceVar);
            signature.add(ValueType.object(method.getOwnerName()));
        }
        for (int i = 0; i < method.parameterCount(); ++i) {
            constructorInvocation.getArguments().add(program.createVariable());
        ...
        suspendInvocation.setMethod(new MethodReference(Fiber.class, "suspend", Fiber.AsyncCall.class,
                Object.class));
        suspendInvocation.getArguments().add(newInstruction.getReceiver());
